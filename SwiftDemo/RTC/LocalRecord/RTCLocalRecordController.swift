...
class RTCLocalRecordController: NSViewController {
    ...
    override func prepare(for segue: NSStoryboardSegue, sender: Any?) {
        guard segue.identifier == "enterRTCLocalRecord" else {
            return
        }
        ...
        guard let nextWindowExistsController = segue.destinationController as? NSWindowController,
              let rtcNextViewController = nextWindowExistsController.contentViewController as? RTCLocalRecordViewController
        else {
            return
        }

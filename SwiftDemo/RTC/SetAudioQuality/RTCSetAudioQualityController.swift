//
//  RTCSetAudioQualityController.swift
//  TRTCSimpleDemo
//
...
class RTCSetAudioQualityController: NSViewController {
    ...
    override func prepare(for segue: NSStoryboardSegue, sender: Any?) {
        guard segue.identifier == "enterRTCSetAudioRoom" else {
            return
        }
        ...
        guard let nextWindowExistsController = segue.destinationController as? NSWindowController,
              let rtcNextViewController = nextWindowExistsController.contentViewController as? RTCSetAudioQualityViewController
        else {
            return
        }

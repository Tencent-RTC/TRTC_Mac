//
//  RTCSetBackgroundMusicController.swift
//  TRTCSimpleDemo
//
...
class RTCSetBackgroundMusicController: NSViewController {
    ...
    override func prepare(for segue: NSStoryboardSegue, sender: Any?) {
        guard segue.identifier == "enterRTCBackgroundMusic" else {
            return
        }
        ...
        guard let nextWindowExistsController = segue.destinationController as? NSWindowController,
              let rtcNextViewController = nextWindowExistsController.contentViewController as? RTCSetBackgroundMusicViewController
        else {
            return
        }
